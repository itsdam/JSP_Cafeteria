...
<body style="background-color : White; font-family: 'Gowun Dodum', sans-serif;">
	...
	<p class="info4"><strong>TODAY</strong> <%=strdate%></p>
		...
		<form method="post" class="pull-right" action="srchcafeteria.jsp">
			<fieldset style="margin-right: 175px;">
                   <label>검색분류</label>
                       <select name = "srchKey">
	...
	<br>
		...
		<div class="row">
			<table class="table table-striped" style="text-align: right; border: 1px solid #dddddd">
				...
				<tbody style="background-color : white;">
					...
					<tr>
						...
						<td style="text-align: center;"><a href="cafeteriaView.jsp?cafeteriaID=<%= list.get(i).getCafeteriaID() %>"><%= list.get(i).getCafeteriaTitle() %></a></td>
						<td style="text-align: center;"><%= list.get(i).getMemberID() %></td>
						<td style="text-align: center;"><%= list.get(i).getCafeteriaDate().substring(0, 11) + list.get(i).getCafeteriaDate().substring(11, 13) + ":" + list.get(i).getCafeteriaDate().substring(14, 16)%></td>
					</tr>
					<%
						}
					%>
			...
			%>
			<a href="main.jsp" class="button pull-right">메인으로</a>
			<% if (memberID != null) { 	%>
